...
__all__ = ["get_monai_conf", "get_omega_conf", "get_conf", "monai_parse_cfg"]
...
def get_monai_conf(conf_file, parse_cfg=True):
    """
    ...
    if parse_cfg:
        ...
        conf = edict(yaml.load(open(conf_file), Loader=yaml.FullLoader))
    return conf
...
def get_conf(conf_file, conf_parser="monai", rank=0, parse_cfg=True):
    conf_file = Path(conf_file)
    assert conf_parser in ['monai', 'omega', 'auto'], f"Config parser {conf_parser} not supported!"

    if conf_parser == 'monai':
        conf = get_monai_conf(conf_file, parse_cfg=parse_cfg)
    ...
    if rank == 0:
        ...
        if getattr(conf, "save_cfg", True):
            # conf_file.link_to(cfg_bak_path) 
            shutil.copy(conf_file, cfg_bak_path)
    return conf

def monai_parse_cfg(conf: dict):
    parser = monai.bundle.ConfigParser(conf)
